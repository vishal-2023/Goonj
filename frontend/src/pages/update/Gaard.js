...
const Gaard = () => {
    ...
    const PageContent = ({ content }) => (
        <div className='flex flex-col gap-4 flex-wrap text-wrap w-10/12'>
  ...
  return (
    <div className='overflow-hidden lazy-loading'>
        ...
        <div className='w-11/12 mx-auto flex flex-row flex-wrap'>
            <div className='w-1/2 flex flex-wrap gap-3 justify-center'>
                {
                    Pages.map((item)=>(
                        <div key={item.id} className={`border-2 rounded-lg hover:scale-95 hover:border-red-500 w-60 p-10 ${item.color}`} onClick={()=>HandlerClick(item.id)} >
                            <img className='w-10 mx-auto' src={item.imgdata} alt='' />
        ...
        <div className='w-9/12 mx-auto flex flex-col gap-7'>
            <div className='text-center text-4xl mt-10 font-bold'>Unique Approaches</div>
            <div className='border-b-4 w-1/2 border-red-600 mx-auto mb-10'></div>
            <ol className='w-full flex flex-col gap-2 text-lg font-extrathin'>
